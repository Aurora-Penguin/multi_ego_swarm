...
namespace ego_planner
{
      ...
      const bool flag_randomPolyTraj, const bool touch_goal)
  {
    ...
    {
      ROS_WARN("[ReplanStage] computeInitState failed. flag_polyInit=%d flag_randomPolyTraj=%d start=(%.2f, %.2f, %.2f) target=(%.2f, %.2f, %.2f)",
    ...
    {
      ROS_WARN("[ReplanStage] finelyCheckAndSetConstraintPoints failed. flag_polyInit=%d flag_randomPolyTraj=%d start=(%.2f, %.2f, %.2f) target=(%.2f, %.2f, %.2f)",
               flag_polyInit, flag_randomPolyTraj,
               start_pt(0), start_pt(1), start_pt(2),
               local_target_pt(0), local_target_pt(1), local_target_pt(2));
      return false;
    ...
    if (!flag_success)
    {
      ROS_WARN("[ReplanStage] optimizeTrajectory failed. flag_polyInit=%d flag_randomPolyTraj=%d segments=%zu start=(%.2f, %.2f, %.2f) target=(%.2f, %.2f, %.2f)",
               flag_polyInit, flag_randomPolyTraj, segments.size(),
               start_pt(0), start_pt(1), start_pt(2),
               local_target_pt(0), local_target_pt(1), local_target_pt(2));
    }
      ...
      Eigen::Vector3d &local_target_vel, bool &touch_goal)
  {
    double t;
    touch_goal = false;
    const double local_target_dist = std::max(1.0, planning_horizen - 1.0);

    traj_.global_traj.last_glb_t_of_lc_tgt = traj_.global_traj.glb_t_of_lc_tgt;

    double t_step = local_target_dist / 20 / pp_.max_vel_;
