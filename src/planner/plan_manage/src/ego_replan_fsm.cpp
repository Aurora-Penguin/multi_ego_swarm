...
namespace ego_planner
{
  ...
  void EGOReplanFSM::init(ros::NodeHandle &nh)
  {
    ...
    nh.param("fsm/backward_goal_dot_thresh", backward_goal_dot_thresh_, -0.15);
    nh.param("fsm/backward_horizon_scale", backward_horizon_scale_, 0.35);
  ...
  bool EGOReplanFSM::callReboundReplan(bool flag_use_poly_init, bool flag_randomPolyTraj)
  {
    ...
    const Eigen::Vector3d target_vec = local_target_pt_ - start_pt_;
    const double target_dist = target_vec.norm();
    if (target_dist > 1e-3)
    {
      const double heading_dot = body_forward_dir_.dot(target_vec / target_dist);
      if (heading_dot < backward_goal_dot_thresh_)
      {
        const double min_backward_horizon = std::max(1.0, planning_horizen_ * backward_horizon_scale_);
        if (target_dist > min_backward_horizon)
        {
          local_target_pt_ = start_pt_ + target_vec / target_dist * min_backward_horizon;
          local_target_vel_.setZero();
          touch_goal_ = false;
          ROS_INFO("[PlanInput] backward target detected, shrink local horizon from %.2f to %.2f m (heading_dot=%.2f)",
                   target_dist, min_backward_horizon, heading_dot);
        }
      }
    }

    const int local_target_occ = planner_manager_->grid_map_->getInflateOccupancy(local_target_pt_);
    ROS_INFO("[PlanInput] start=(%.2f, %.2f, %.2f) local_target=(%.2f, %.2f, %.2f) final_goal=(%.2f, %.2f, %.2f) touch_goal=%d local_target_occ=%d",
    ...
    if (!plan_success)
    {
      ROS_WARN("[PlanFail] start=(%.2f, %.2f, %.2f) local_target=(%.2f, %.2f, %.2f) final_goal=(%.2f, %.2f, %.2f) touch_goal=%d local_target_occ=%d",
               start_pt_(0), start_pt_(1), start_pt_(2),
               local_target_pt_(0), local_target_pt_(1), local_target_pt_(2),
               final_goal_(0), final_goal_(1), final_goal_(2),
               touch_goal_, local_target_occ);
  ...
  void EGOReplanFSM::waypointCallback(const quadrotor_msgs::GoalSetPtr &msg)
  {
    ...
    if (!have_odom_)
    {
      ROS_WARN("Received a goal but odometry is not ready yet, ignore this goal.");
      return;
    }

    ROS_INFO("Received goal for drone %d: %f, %f, %f", msg->drone_id, msg->goal[0], msg->goal[1], msg->goal[2]);
  ...
  void EGOReplanFSM::odometryCallback(const nav_msgs::OdometryConstPtr &msg)
  {
    ...
    const Eigen::Quaterniond body_q(msg->pose.pose.orientation.w,
                                    msg->pose.pose.orientation.x,
                                    msg->pose.pose.orientation.y,
                                    msg->pose.pose.orientation.z);
    body_forward_dir_ = body_q.toRotationMatrix().col(0);
