...
namespace ego_planner
{
      ...
      const bool flag_first_init /*= true*/)
  {
    ...
    for (size_t i = 0; i < segment_ids.size(); i++)
    {
      ...
      //step 3只要这个段里至少有一个点成功找到了交点和方向，那么前后没找到方向的点，就继承最近邻点的方向
      if (got_intersection_id >= 0)
      {
  ...
  bool PolyTrajOptimizer::roughlyCheckConstraintPoints(void)
  {
    ...
    for (int i = 1; i <= i_end; ++i)
    {
      ...
      if (occ)
      {
        ...
        {
          const Eigen::Vector3d local_target = cps_.points.col(std::min(i, cps_.cp_size - 1));
          ROS_WARN("Local target in collision, skip this planning. cp_size=%d check_id=%d local_target=(%.2f, %.2f, %.2f)",
                   cps_.cp_size, i, local_target(0), local_target(1), local_target(2));
